...
const isReservedTag = makeMap('template,script,style,element,content,slot,link,meta,svg,view,' +
  ...
  'video,web,embed,tabbar,tabheader,datepicker,timepicker,marquee,countdown', true)
...
## Vue选项合并的时候，为什么将data选项变为函数?

1. 通过函数返回数据对象，保证了每个组件实例都有一个唯一的数据副本，避免了组件间数据互相影响。`Vue`在初始化`data`的时候就是调用`strats.data`函数来获取组件数据。  
2. 将data选项变为函数就可以自定义获取data的时间。`props`、`inject `是在`data`之前初始化的，目的就是让`data`能够使用其中的数据。